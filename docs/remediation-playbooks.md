...
- Integer return value (AWS API requires integer concurrency values; the result is coerced via `math.ceil`).
...
- Input validation: a negative `avg_rps` or `current_throttles` raises `ValueError` so the agent never acts on a malformed metric reading.
