...
- Integer return value via `math.ceil` (AWS API requires integer concurrency values).
...
- Input validation: negative `avg_rps` or `current_throttles` raise `ValueError` rather than producing a nonsensical recommendation.
