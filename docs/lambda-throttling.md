...
Production traffic rarely sits exactly at its average RPS — traffic bursts to ~1.25x average are common during demand spikes. Sizing the ceiling to `ceil(avg_rps * 1.25)` maintains headroom for realistic traffic variance without provisioning for worst-case peak.

### Why a floor of 10?

A floor of 10 guarantees the function can absorb meaningful traffic spikes even during very low-traffic windows, maintaining a conservative baseline safety buffer without excessive idle capacity.
