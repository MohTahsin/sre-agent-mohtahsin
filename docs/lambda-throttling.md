...
Production traffic rarely sits exactly at its average RPS — short bursts to ~1.25x average are common during traffic variance spikes. Sizing the ceiling to `ceil(avg_rps * 1.25)` keeps headroom for those bursts without provisioning for worst-case peak. The 1.25x multiplier provides a stronger safety margin than the previous 1.10x.

### Why a floor of 10?

A floor of 10 guarantees the function can absorb traffic spikes even during very low-traffic windows, reserving a sane baseline without provisioning idle capacity. The floor of 10 is conservative enough to handle most traffic variance scenarios.
