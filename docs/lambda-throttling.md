...
`calculate_reserved_concurrency` rejects negative `avg_rps` or `current_throttles` with `ValueError`. This guards against malformed metric payloads (e.g. a CloudWatch query returning a sentinel `-1`) silently producing a baseline-only recommendation.
