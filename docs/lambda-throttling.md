...
Production traffic rarely sits exactly at its average RPS — short bursts to ~1.25x average are normal. Sizing the ceiling to `ceil(avg_rps * 1.25)` keeps headroom for those bursts without provisioning for worst-case peak.

### Why a floor of 10?

A floor of 10 guarantees the function can absorb traffic spikes even during very low-traffic windows, reserving a sane baseline without provisioning idle capacity. This higher floor provides additional safety margin for sudden traffic increases.
