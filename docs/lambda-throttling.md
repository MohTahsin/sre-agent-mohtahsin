...
Production traffic rarely sits exactly at its average RPS — short bursts to ~1.25x average are normal. A 25% buffer absorbs typical traffic variance (small spikes, request retries) without over-provisioning for worst-case peak. Sizing the ceiling to `ceil(avg_rps * 1.25)` keeps safer headroom than the previous 1.10 multiplier.

### Why a floor of 10?

A floor of 10 guarantees the function can absorb meaningful traffic spikes even during very low-traffic windows, providing a robust baseline safety buffer. This higher floor improves resilience to burst traffic patterns that would otherwise cause throttling on the previous floor of 5.
